...
pub fn register_speech() {
    let model_path = "vosk_models/vosk-model-small-fr-0.22"; //args.next().expect("A model path was not provided");
    ...
    let mut recognizer = Recognizer::new(&model, config.sample_rate().0 as f32).expect(
        ...
        match config.sample_format() {
            SampleFormat::F32 =>
                audio_input_device.build_input_stream(
                    &config.into(),
                    move |data: &[f32], _|
                        recognize(&mut recognizer_clone.lock().unwrap(), data, channels),
                    err_fn,
...
pub fn recognize<T: Sample + ToSample<i16>>(
    ...
    let data: Vec<i16> = data
        ...
        .map(|v| v.to_sample())
        .collect();
    let data = if channels != 1 { stereo_to_mono(&data) } else { data };
...
pub fn stereo_to_mono(input_data: &[i16]) -> Vec<i16> {
    ...
    result.extend(input_data.chunks_exact(2).map(|chunk| chunk[0] / 2 + chunk[1] / 2));
